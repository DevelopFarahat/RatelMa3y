    ...
    return(
        ...
        <div className={PostDetailsStyles['post-details-main-container']} style={{direction: postDetails?.lang === 'ar'? "rtl": 'ltr'}}>
            ...
            <div className={PostDetailsStyles['post-details-title']}>
                {postDetails.title}
                {postDetails.latest?<span className={PostDetailsStyles['latest-post-badge']}>{t('events_latest')}<TbUrgent style={{marginBottom: '5px'}}/></span>:null}
            </div>
            ...
            <button type="button" style={{direction: 'ltr'}} className={PostDetailsStyles['btn']} onClick={()=> navigate(-1)}><IoChevronBack size={20} style={{marginTop:'-3px'}}/>{t('events_back')}</button>
        </div>
        </>
    )
}
