    ...
    return (
        ...
        {props.post.id === props.latestPost.id?props.post.latest = true:null}
            <div className={PostStyles['post-main-container']}>
                ...
                {props.post.details.split(" ").length > 63 ? <p className={PostStyles['post-details']}>{props.post.details.substring(0, 374)}<Link to={`/postDetails/${props.post.id}`}  state={props.post} className={PostStyles['read-more']}>Read More....</Link></p> : <p className={PostStyles['post-details']}>{props.post.details}</p>}
