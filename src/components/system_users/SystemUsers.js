...
const SystemUsers = () => {
    ...
    const errorHandle = (filed, value) => {
        if (filed === 'username') {
            ...
            const mobileRegx = /^(01)(0|1|2|5)[0-9]{8}$/;
            setErrors({
                ...errors,
                mobileError: value.length === 0 ? 'Mobile Is Required' : mobileRegx.test(value) ? null : 'Mobile Must Start With 01 and consists of 11 digit'
            });
    ...
    const filterAccounts = () => {
        ...
        for (let i = 0; i < systemDataAccountsArr.length; i++) {
            if (filterValue === systemDataAccountsArr[i].username) {
                filtarationArr.push(systemDataAccountsArr[i]);
            } else if (filterValue === systemDataAccountsArr[i].fullname) {
                filtarationArr.push(systemDataAccountsArr[i]);
            }
            else if (filterValue === systemDataAccountsArr[i].privilege) {
                filtarationArr.push(systemDataAccountsArr[i]);
            }
        }
        ...
        <>
            ...
            <form className={SystemUsersStyles['system-user-form']} method="post" onSubmit={handleSubmit}>
                <div>
                    ...
                    <Form.Control type="text" name="Username" id="username" value={userData.username} onChange={handleChange} className={`${SystemUsersStyles['system-user-form-controls']} ${errors.usernameError ? 'border-danger' : ''}`} />
                    <small className='text-danger'>{errors.usernameError}</small>
                </div>
                <div>
                    <Form.Label htmlFor="fullname">Fullname</Form.Label>
                    <Form.Control type="text" name="Fullname" id="fullname" value={userData.fullname} onChange={handleChange} className={`${SystemUsersStyles['system-user-form-controls']} ${errors.fullnameError ? 'border-danger' : ''}`} />
                    <small className='text-danger'>{errors.fullnameError}</small>
                </div>
                <div>
                    <Form.Label htmlFor="password">Password</Form.Label>
                    <Form.Control type="password" name="Password" id="password" value={userData.password} onChange={handleChange} className={`${SystemUsersStyles['system-user-form-controls']} ${errors.passwordError ? 'border-danger' : ''}`} />
                    <small className='text-danger'>{errors.passwordError}</small>
                </div>
                <div>
                    <Form.Label htmlFor="mobile">Mobile</Form.Label>
                    <Form.Control type="tel" name="Mobile" id="mobile" value={userData.mobile} onChange={handleChange} className={`${SystemUsersStyles['system-user-form-controls']} ${errors.mobileError ? 'border-danger' : ''}`} />
                    <small className='text-danger'>{errors.mobileError}</small>
                </div>
                <div>
                    <Form.Label htmlFor="privilege">Privilege</Form.Label>
                    <Form.Select id="privilege" name="Privilege" onChange={handleChange} className={`${SystemUsersStyles['system-user-form-controls']} ${errors.privilegeError ? 'border-danger' : ''}`} >
            ...
            <div className={SystemUsersStyles['system-user-data-container']}>
                ...
                <div className={SystemUsersStyles['table-wrapper']}>
                    <table className={SystemUsersStyles['system-accounts-table']}>
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Username</th>
