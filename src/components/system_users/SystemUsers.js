...
const SystemUsers = () => {
    ...
    return (
       ...
       {/*<Pagination currentPage={setCurrentPage} setCurrentPage={setCurrentPage} lastPage={lastPage} pageNoArrLength={pageNoArrLength} pageNoCopy={pageNoCopy} setPageNoCopy={setPageNoCopy} pageNo={pageNo} setPageNo={setPageNo}/> */} 

       <div className={SystemUsersStyles['pagination-container']} style={{direction:'rtl'}}>
      { 
     pageNo !== undefined?<ul>
      <button type="button" className={SystemUsersStyles['btn']} disabled={currentPage === 1?true:false} style={{cursor: 1 === currentPage?'not-allowed':'pointer',opacity:1 === currentPage?'.5':'1'  }}  onClick={getThePreviousPages}>
     <img src={currentPage === 1?GreaterThanGrayImage:GreaterThanWhiteImage} alt="GreaterThan"/>
      ...
      {lastPage !== undefined? lastPage.index !== -1?
      <li id={lastPage.id} style={{background:Number(currentPage)  === lastPage.index  ?'#c2a054':'',color:Number(currentPage)  === lastPage.index?'#FFFFFF':''}}  onClick={handleUpCommingPage}>{lastPage.index}</li>:null:null}
      
      <button type="button" className={SystemUsersStyles['btn']} disabled={currentPage === lastPage.index?true:false} style={{cursor:currentPage === lastPage.index?'not-allowed':'pointer',opacity:currentPage === lastPage.index?'.5':'1' }}  onClick={getTheNextPages}>
