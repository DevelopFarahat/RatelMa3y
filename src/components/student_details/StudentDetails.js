...
const StudentDetails = ({
  ...
  isStudentRequestDataVisible,
  isStudentRatelDataVisible,
  setIsStudentRequestDataVisible,
  setIsStudentRatelDataVisible,
}) => {
  let programes = [
    { id: 0, name: "programprogr1" },
    { id: 1, name: "programprogr2" },
    { id: 2, name: "programprogr3" },
  ];
  let days = [
    "Sunday",
    "Monday",
  ...
  let working_hours = [
    { id: 0, hour: "from 2:30 pm to 5:30 pm" },
    { id: 1, hour: "from 2:30 pm to 5:30 pm" },
    { id: 2, hour: "from 2:30 pm to 5:30 pm" },
    { id: 3, hour: "from 2:30 pm to 5:30 pm" },
  ];
  ...
  let sessions = [
    { _id: 0, is_live: false, created_at: "2022-08-14T12:07:43.020Z" },
    { _id: 1, is_live: true, created_at: "2022-08-16T12:07:43.020Z" },
    { _id: 2, is_live: true, created_at: "2022-07-14T12:07:43.020Z" },
    { _id: 3, is_live: true, created_at: "2022-08-19T12:07:43.020Z" },
    { _id: 4, is_live: false, created_at: "2022-05-1T12:07:43.020Z" },
  ];
  const [sess, setSessions] = useState(
    specificStudentJoiningRequestData?.sessions
  );
  ...
  const sortSessions = (event) => {
    ...
    for (
      let i = 0;
      i < specificStudentJoiningRequestData.sessions.length;
      i++
    ) {
      console.log(
        specificStudentJoiningRequestData.sessions[i].attendants.length === 0
      );
      if (specificStudentJoiningRequestData.sessions[i].attendants.length) {
        for (
          let j = 0;
          j < specificStudentJoiningRequestData.sessions[i].attendants.length;
          j++
        ) {
          if (
            specificStudentJoiningRequestData._id ===
            specificStudentJoiningRequestData.sessions[i].attendants[j]
          ) {
            console.log("yes true");
            specificStudentJoiningRequestData.sessions[
              i
            ].isStudentPersent = true;
            sortedSessionsArr.push(
              specificStudentJoiningRequestData.sessions[i]
            );
          } else {
            specificStudentJoiningRequestData.sessions[
              i
            ].isStudentPersent = false;
            sortedSessionsArr.push(
              specificStudentJoiningRequestData.sessions[i]
            );
          }
    ...
    */
    ...
    setSessions(sortedSessionsArr);
  };

  const resetSortSessions = () => {
    setSpecificStudentJoiningRequestData(
      initialSpecificStudentJoiningRequestData.current
    );
  };
  ...
  return (
    <>
      ...
      <div className={StudentDetailsStyles["student-details-main-container"]}>
        {Object.keys(specificStudentJoiningRequestData).length === 0 ? (
          <img
            src={EmptyDataImage}
            className={StudentDetailsStyles["empty-data-img"]}
            alt="Empty"
          />
        ) : specificStudentJoiningRequestData.subscription_state !==
          "Cancelled" ? (
          <div className={StudentDetailsStyles["settings-header"]}>
            <span
              className={`${
                isStudentRequestDataVisible
                  ? StudentDetailsStyles["taps-shadow"]
                  : ""
              } ${StudentDetailsStyles["joinnig-request-data-tap"]}`}
              onClick={toogleViewOfStudentRatelData}
            >
              Joinnig Request Data
            </span>{" "}
            {specificStudentJoiningRequestData.subscription_state !==
            "Pending" ? (
              <span
                className={`${
                  isStudentRatelDataVisible
                    ? StudentDetailsStyles["taps-shadow"]
                    : ""
                } ${StudentDetailsStyles["ratel-student-data-tap"]}`}
                onClick={toogleViewOfStudentRequestData}
              >
                Ratel Student Data
              </span>
            ) : null}
        ...
        {Object.keys(specificStudentJoiningRequestData).length !== 0 &&
        specificStudentJoiningRequestData.subscription_state !== "Cancelled" ? (
          isStudentRequestDataVisible &&
          specificStudentJoiningRequestData.subscription_state !==
            "Cancelled" ? (
            <div
              className={
                StudentDetailsStyles[
                  "student-request-joinnig-info-main-container"
                ]
              }
            >
              {/* request */}
              <table
                className={
                  StudentDetailsStyles["requested-joinnig-student-table"]
                }
              >
                <thead>
                  <tr>
                    ...
                    {specificStudentJoiningRequestData.subscription_state !==
                      "Pending" &&
                    specificStudentJoiningRequestData.instructor.age !==
                      null ? (
                  ...
                  <tr>
                    ...
                    {specificStudentJoiningRequestData.subscription_state !==
                      "Pending" &&
                    specificStudentJoiningRequestData.instructor.name !==
                      null ? (
                      <>
                        <th>Name</th>
                        <td>
                  ...
                  <tr>
                    ...
                    {specificStudentJoiningRequestData.subscription_state !==
                      "Pending" &&
                    specificStudentJoiningRequestData.instructor.gender !==
                      null ? (
                      <>
                        <th>Gender</th>
                        <td>
                  ...
                  <tr>
                    ...
                    {specificStudentJoiningRequestData.subscription_state !==
                      "Pending" &&
                    specificStudentJoiningRequestData.instructor.state !==
                      null ? (
                      <>
                        <th>State</th>
                        <td>
                  ...
                  <tr>
                    {!specificStudentJoiningRequestData.program_prefs.type ? (
                      <td>{""}</td>
                    ) : (
                      <td>
                        <span>
                    ...
                    {!specificStudentJoiningRequestData.program_prefs
                      .pref_days ? (
                      <td>{""}</td>
                    ) : (
                      <td>
                    ...
                    {!specificStudentJoiningRequestData.program_prefs
                      ...
                      <td>
                        <span>
                          {specificStudentJoiningRequestData.program_prefs.pref_times_of_day.map(
                            (prefTimeD, index) =>
                              prefTimeD[1] !== 0 ? (
                                <div key={index}>
                                  <span>from</span>{" "}
                                  <span>
                                    {workingHoursArr[index][prefTimeD[0]]}
                                  </span>{" "}
                                  <span>to</span>{" "}
                                  <span>
                                    {workingHoursArr[index][prefTimeD[1]]}
                                  </span>
                                </div>
                              ) : null
                          )}
          ...
          ) : specificStudentJoiningRequestData.sessions.length !== 0 ? (
            <div
              className={
                StudentDetailsStyles["student-ratel-info-main-container"]
              }
            >
              {/* ratel member */}
              ...
              <div className={StudentDetailsStyles["table-settings-container"]}>
                ...
                <Form.Select
                  name="sort-student-sessions"
                  id="sortStudentSessions"
                  className={StudentDetailsStyles["form-sort-select"]}
                ...
                <button
                  type="button"
                  className={StudentDetailsStyles["btn"]}
                  onClick={resetSortSessions}
                >
                  Reset
                  <BiReset />
                </button>
              ...
              <div className={StudentDetailsStyles["table-wrapper"]}>
                {specificStudentJoiningRequestData.sessions.length === 0 ? (
                  <img
                    ...
                    className={StudentDetailsStyles["no-result"]}
                    alt="no-result"
                  />
                ) : (
                  <table
                    className={
                      StudentDetailsStyles["student-session-info-table"]
                    }
                  >
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {specificStudentJoiningRequestData.sessions !== null &&
                      specificStudentJoiningRequestData.sessions !== undefined
                        ? specificStudentJoiningRequestData.sessions.map(
                            (session) => (
                              <tr key={session._id} id={session._id}>
                                {session.attendants.length !== 0 &&
                                session.attendants !== null &&
                                session.attendants !== undefined ? (
                                  <td>
                                    {session.attendants.map((atten) =>
                                      specificStudentJoiningRequestData._id ===
                                      atten ? (
                                        <img
                                          src={Present}
